...
public class InterfaceClient implements IInterfaceClient {
    ...
    public List<String> getAllLanguages() {
        ...
        Minecraft.getInstance().getLanguageManager().getLanguages().forEach((language, info) -> list.add(language));
        return list;
    }
    ...
    public String getFluidName(String fluidID) {
        for (Entry<ResourceKey<Fluid>, Fluid> fluidEntry : ForgeRegistries.FLUIDS.getEntries()) {
            if (fluidEntry.getKey().location().getPath().equals(fluidID)) {
                return Component.translatable(fluidEntry.getValue().getFluidType().getDescriptionId()).getString();
            }
        }
        return "INVALID";
    }
