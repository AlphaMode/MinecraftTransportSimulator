...
public class InterfaceEventsEntityRendering {
    ...
    public static void onIVCameraSetup(ComputeCameraAngles event) {
        ...
        if (camera.getEntity() instanceof Player plr) {
            mcPlayer = plr;
    ...
    public static void onIVRenderOverlayPre(RenderGuiOverlayEvent.Pre event) {
        ...
        if ((event.getOverlay().id().equals(VanillaGuiOverlay.CROSSHAIR.id()) || event.getOverlay().id().equals(VanillaGuiOverlay.HOTBAR.id())) && CameraSystem.customCameraOverlay != null) {
            event.setCanceled(true);
            return;
        }

        //If we are seated in a controller seat, and are rendering GUIs, disable the hotbar.
        if ((event.getOverlay().id().equals(VanillaGuiOverlay.HOTBAR.id()) || event.getOverlay().id().equals(VanillaGuiOverlay.FOOD_LEVEL.id()) || event.getOverlay().id().equals(VanillaGuiOverlay.PLAYER_HEALTH.id()) || event.getOverlay().id().equals(VanillaGuiOverlay.ARMOR_LEVEL.id()) || event.getOverlay().id().equals(VanillaGuiOverlay.EXPERIENCE_BAR.id())) && (InterfaceManager.clientInterface.getCameraMode() == CameraMode.FIRST_PERSON ? ConfigSystem.client.renderingSettings.renderHUD_1P.value : ConfigSystem.client.renderingSettings.renderHUD_3P.value)) {
        ...
        if (event.getOverlay().id().equals(VanillaGuiOverlay.CHAT_PANEL)) {
            //Set up variables.
